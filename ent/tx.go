...
func (tx *Tx) init() {
	tx.EntForm = NewEntFormClient(tx.config)
	tx.EntFormQuestion = NewEntFormQuestionClient(tx.config)
	tx.FormInstance = NewFormInstanceClient(tx.config)
	tx.FormSpec = NewFormSpecClient(tx.config)
	tx.Question = NewQuestionClient(tx.config)
	tx.QuestionGroup = NewQuestionGroupClient(tx.config)
	tx.QuestionResponse = NewQuestionResponseClient(tx.config)
...
// applies a query, for example: EntForm.QueryXXX(), the query will be executed
