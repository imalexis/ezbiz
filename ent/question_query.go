...
type QuestionQuery struct {
	...
	withFromQuestionGroup     *QuestionGroupQuery
...
// QueryFromQuestionGroup chains the current query on the "from_question_group" edge.
func (qq *QuestionQuery) QueryFromQuestionGroup() *QuestionGroupQuery {
	query := (&QuestionGroupClient{config: qq.config}).Query()
...
func (qq *QuestionQuery) QueryQuestionResponse() *QuestionResponseQuery {
	query := (&QuestionResponseClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) Clone() *QuestionQuery {
	...
	return &QuestionQuery{
		...
		withFromQuestionGroup: qq.withFromQuestionGroup.Clone(),
		withQuestionResponse:  qq.withQuestionResponse.Clone(),
...
// WithFromQuestionGroup tells the query-builder to eager-load the nodes that are connected to
// the "from_question_group" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithFromQuestionGroup(opts ...func(*QuestionGroupQuery)) *QuestionQuery {
	query := (&QuestionGroupClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withFromQuestionGroup = query
	return qq
}
...
func (qq *QuestionQuery) WithQuestionResponse(opts ...func(*QuestionResponseQuery)) *QuestionQuery {
	query := (&QuestionResponseClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withQuestionResponse = query
	return qq
}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	var (
		...
		_spec       = qq.querySpec()
		loadedTypes = [2]bool{
			qq.withFromQuestionGroup != nil,
			qq.withQuestionResponse != nil,
		}
	)
	if qq.withFromQuestionGroup != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := qq.withFromQuestionGroup; query != nil {
		if err := qq.loadFromQuestionGroup(ctx, query, nodes, nil,
			func(n *Question, e *QuestionGroup) { n.Edges.FromQuestionGroup = e }); err != nil {
			return nil, err
		}
	}
	if query := qq.withQuestionResponse; query != nil {
		if err := qq.loadQuestionResponse(ctx, query, nodes,
			func(n *Question) { n.Edges.QuestionResponse = []*QuestionResponse{} },
			func(n *Question, e *QuestionResponse) { n.Edges.QuestionResponse = append(n.Edges.QuestionResponse, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range qq.withNamedQuestionResponse {
		if err := qq.loadQuestionResponse(ctx, query, nodes,
			func(n *Question) { n.appendNamedQuestionResponse(name) },
			func(n *Question, e *QuestionResponse) { n.appendNamedQuestionResponse(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (qq *QuestionQuery) loadFromQuestionGroup(ctx context.Context, query *QuestionGroupQuery, nodes []*Question, init func(*Question), assign func(*Question, *QuestionGroup)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Question)
	for i := range nodes {
		if nodes[i].question_group_question == nil {
			continue
		}
		fk := *nodes[i].question_group_question
	...
	query.Where(questiongroup.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "question_group_question" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (qq *QuestionQuery) loadQuestionResponse(ctx context.Context, query *QuestionResponseQuery, nodes []*Question, init func(*Question), assign func(*Question, *QuestionResponse)) error {
	...
	query.Where(predicate.QuestionResponse(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.QuestionResponseColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.question_question_response
		if fk == nil {
			return fmt.Errorf(`foreign-key "question_question_response" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "question_question_response" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (qq *QuestionQuery) WithNamedQuestionResponse(name string, opts ...func(*QuestionResponseQuery)) *QuestionQuery {
	query := (&QuestionResponseClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if qq.withNamedQuestionResponse == nil {
		qq.withNamedQuestionResponse = make(map[string]*QuestionResponseQuery)
	}
	qq.withNamedQuestionResponse[name] = query
	return qq
}
