...
func (uc *UserCreate) AddFormSpecIDs(ids ...int) *UserCreate {
	uc.mutation.AddFormSpecIDs(ids...)
	return uc
}

// AddFormSpecs adds the "form_specs" edges to the FormSpec entity.
func (uc *UserCreate) AddFormSpecs(f ...*FormSpec) *UserCreate {
	ids := make([]int, len(f))
	for i := range f {
		ids[i] = f[i].ID
	}
	return uc.AddFormSpecIDs(ids...)
}

// AddFormInstanceIDs adds the "form_instances" edge to the FormInstance entity by IDs.
func (uc *UserCreate) AddFormInstanceIDs(ids ...int) *UserCreate {
	uc.mutation.AddFormInstanceIDs(ids...)
	return uc
}

// AddFormInstances adds the "form_instances" edges to the FormInstance entity.
func (uc *UserCreate) AddFormInstances(f ...*FormInstance) *UserCreate {
	ids := make([]int, len(f))
	for i := range f {
		ids[i] = f[i].ID
	}
	return uc.AddFormInstanceIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.FormSpecsIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.FormInstancesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
